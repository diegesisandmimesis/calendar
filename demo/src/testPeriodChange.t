...
gameMain: GameMainDef
	...
	newGame() {
		gCalendar.subscribe(myNotifier, 'periodChange');
		inherited();
	}
;
...
myNotifier: EventListener
	eventHandler(obj) {
		if((obj.data == nil) || (obj.data.length < 2)) {
			"<.p>Event handler got bad argument.<.p> ";
			return;
		}
		"<.p>The period changed from <<obj.data[1]>>
		to <<obj.data[2]>>.<.p> ";
	}
;
